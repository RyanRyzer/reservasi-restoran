...
class Pembayaran608 {
    ...
    public function ProsesLogin1() {
    session_start();

    $username = array("2313010608", "2313010604", "2313010605");
    $password = array("2313010608", "2313010604", "2313010605");

    if (in_array($_POST['username'], $username) && in_array($_POST['password'], $password)) {
        if ($_POST['username'] === "2313010608" && $_POST['password'] === "2313010608") {
            $_SESSION["Nama"] = "Ryan";
            $_SESSION["NIM"] = "2313010608";
            $_SESSION["Gambar"] = "assets/Ryzer.png";
            header("Location: dashboard.php?page=pembayaran");
            exit();
        } else if ($_POST['username'] === "2313010605" && $_POST['password'] === "2313010605") {
            $_SESSION["Nama"] = "Angga";
            $_SESSION["NIM"] = "2313010605";
            $_SESSION["Gambar"] = "assets/Angga.png";
            header("Location: dashboard.php?page=menu");
            exit();
        } else if ($_POST['username'] === "2313010604" && $_POST['password'] === "2313010604") {
            $_SESSION["Nama"] = "Ani";
            $_SESSION["NIM"] = "2313010604";
            $_SESSION["Gambar"] = "assets/Ani.png";
            header("Location: dashboard.php?page=reservasi");
            exit();
        } else {
            echo "Login tidak dikenali!";
        }
    } else {
...
if (isset($_SESSION["Nama"]) && isset($_SESSION["NIM"]) && isset($_SESSION["Gambar"])) {
    echo "<div class='bg-white shadow p-4 mb-4 rounded'>";
    echo "<p><strong>Nama:</strong> " . $_SESSION["Nama"] . "</p>";
    echo "<p><strong>NIM:</strong> " . $_SESSION["NIM"] . "</p>";
    echo "<img src='assets/Ryzer.png" . $_SESSION["Gambar"] . "' alt='Ryan Robby' class='w-32 h-32 rounded-full mt-2'>";
    echo "</div>";
}
